...
matrix_t s21_create_matrix(int rows, int columns) {
  matrix_t zero;
  if (rows > 0 && columns > 0 &&
      (zero.matrix = s21_memory_allocation(rows, columns)) != NULL) {
    ...
    do {
      ...
      if (c == 'v' && f == 0) {
        ...
        if (digit_ind == 1) {
          ...
          if (v == 1) {
            res_array.coord_array =
                realloc(res_array.coord_array, sizeof(double) * idx++);
        ...
        if (c == '\n' || c == EOF) {
          ...
          res_array.connect_array =
              realloc(res_array.connect_array, sizeof(unsigned int) * (temp_c));
...
vertex_t TurnAroundShift(vertex_t data, double corner, int type) {
  ...
  for (int m = 0, m_e = 0; m < data.col;) {
    matrix_t matrix_A = s21_create_matrix(4, 4),
             matrix_B = s21_create_matrix(4, 1);
    for (int i = 0; i < 4; i++) {
      if (i == 3) {
